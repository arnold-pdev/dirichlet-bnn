...
loss(x, y) = (m(x) - y)'*(m(x) - y) # partial loss
Loss(x, y) = sum(loss.(x,y)) # total loss
opt = Adam()

accuracy(x, y) = sum(abs.(norm.(m.(x)./sum.(m.(x)).-y,1)))
...
for j in 1:3
    ...
    # plot model
    M = [ m([x[i]])[j] for i in eachindex(x) ]
    plot!(x, M, label = "model")

    savefig("model_fit_mse_$(j).png")
end